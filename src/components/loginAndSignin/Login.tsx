  ...
  async function login() {
    try {
      ...
      if (error instanceof Response) {
        ...
        const backErrors = await error.json();
        ...
        setErrors((prevErrors) => ({
          ...prevErrors,
          email:"",
          password:"Oops! It looks like your username or password is incorrect. Double-check and try again.",
        }));
      }
      } else {
  ...
  return (
    <div className="h-full w-full flex-row flex">
        ...
        <div className={`${SigninCss.rightPart}`}>
            <div className={`${SigninCss.creatAcc}`}>Login</div>
            <form className={`${SigninCss.form} flex flex-col gap-4`} action="" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className={`${SigninCss.input_container}`}>
                    ...
                    <label htmlFor="email" className="text-red-600">{errors.password}</label>
                </div>
                <div className={`${SigninCss.submitButtonDiv}`}>
                    <input type="submit" className={`${SigninCss.submitButton}`} value={"Login"} onClick={login}></input>
                </div>
            </form>
...
c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
      ...
      <span>Sign up with Google</span>
            ...
            <div className={`${SigninCss.haveAcc}`}>Don't have an account?<span className={`${SigninCss.login}`} onClick={()=>navigateTo("/signin")}>Creat Account</span></div>
        </div>
    </div>
);
}
